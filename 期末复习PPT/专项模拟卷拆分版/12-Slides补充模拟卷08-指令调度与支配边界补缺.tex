...
\begin{document}
\section*{Slides 后端完整模拟卷 08（题目与完整解析）}
...
\subsubsection*{一、三类依赖与寄存器重命名（20 分）}
...
1. 写出 RAW、WAR、WAW 的定义。 2. 列出全部 RAW 边。 3. 列出全部 WAR 边。 4. 列出全部 WAW 边。 5. 用寄存器重命名消除假依赖，并给出一个合法重排后的序列。

\subsubsection*{二、资源受限 List Scheduling（20 分）}
...
1. 按“若指令在周期 t 发射，则在 t+latency 时结果可用”的规则，计算无限资源下每个结点的最早发射周期。 2. 计算从每个结点到出口的最长延迟，作为调度优先级。 3. 按优先级从大到小进行 List Scheduling，给出每周期发射结果。 4. 写出最终完成周期。 5. 区分依赖约束和资源约束。

\subsubsection*{三、局部调度、全局调度与软件流水（20 分）}
...
1. 写出单次迭代中的主要真实依赖。 2. 说明局部调度能在哪个范围内移动这些指令。 3. 说明全局调度跨基本块移动指令时必须额外考虑什么。 4. 给出把相邻两次迭代的 load 交叠执行的软件流水思路。 5. 说明寄存器压力为什么可能因调度而上升。

\subsubsection*{四、Dom、PostDom 与 Dominance Frontier（20 分）}
...
1. 计算所有结点的 Dom 集和 idom。 2. 写出支配树。 3. 计算所有结点的 PostDom 集和直接后支配者 ipdom。 4. 计算每个结点的 Dominance Frontier。 5. 若变量 \texttt{x} 分别在 B2、B3 中定值，指出 phi 的插入位置。

\subsubsection*{五、循环 SSA 构造与退出 SSA（20 分）}
...
1. 计算 Dom 集和 idom。 2. 计算 \texttt{DF(B)} 和 \texttt{DF(H)}。 3. 把代码转换成 SSA，写出变量版本和 phi。 4. 写出 phi 两个参数分别来自哪条控制流边。 5. 退出 SSA 时，在各前驱边上写出需要插入的复制。

\subsection*{B 按考试标准重写答案}

\subsubsection*{一、三类依赖与寄存器重命名答案}
...
\includegraphics[width=0.82\linewidth]{figures/instruction-dependency-12.png}
\par{\small 指令依赖图}
\end{figure}
...
\subsubsection*{二、资源受限 List Scheduling 答案}
...
\subsubsection*{三、局部调度、全局调度与软件流水答案}
...
\subsubsection*{四、Dom、PostDom 与 Dominance Frontier 答案}

\begin{figure}[H]\centering
\includegraphics[width=0.82\linewidth]{figures/cfg-12-diamond.png}
\par{\small Diamond CFG}
\end{figure}
...
\begin{figure}[H]\centering
\includegraphics[width=0.82\linewidth]{figures/dom-tree-12-diamond.png}
\par{\small 支配树}
\end{figure}
...
\subsubsection*{五、循环 SSA 构造与退出 SSA 答案}
