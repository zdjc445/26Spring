...
\lhead{编译原理完整模拟卷}
\rhead{\thepage}
\title{Slides 过程间与指针完整模拟卷 10（题目与完整解析）}
\author{}
\date{}
\begin{document}
\maketitle
...
\subsection*{一、调用图、函数摘要与上下文敏感（20 分）}
\addcontentsline{toc}{subsection}{一、调用图、函数摘要与上下文敏感（20 分）}
...
\noindent 1. 构造调用图。\\
\noindent 2. 分别写出 C1、C2 的实际返回值以及 \texttt{main} 的返回值。\\
\noindent 3. 若上下文不敏感分析把 \texttt{f} 的参数合并为 \texttt{\{1,-1\}}，写出它对 C1、C2 返回值的保守结果。\\
\noindent 4. 写出区分 C1、C2 后的上下文敏感结果。\\
\noindent 5. 为 \texttt{g} 和 \texttt{f} 分别写出一个函数摘要。\\

\subsection*{二、调用串与 CFL Reachability（20 分）}
\addcontentsline{toc}{subsection}{二、调用串与 CFL Reachability（20 分）}
...
\noindent 1. 调用串如何表示到达 \texttt{g} 的上下文。\\
\noindent 2. 为什么递归程序必须限制调用串长度。\\
\noindent 3. 如何把 call/return 标号解释成 Dyck 语言括号。\\

\subsection*{三、指针别名分析基本概念（20 分）}
\addcontentsline{toc}{subsection}{三、指针别名分析基本概念（20 分）}


回答下列问题：

\noindent 1. 什么是 alias。\\
\noindent 2. 什么是 may-alias 和 must-alias。\\
\noindent 3. 什么是 points-to set。\\
\noindent 4. flow-sensitive pointer analysis 和 flow-insensitive pointer analysis 有什么区别。\\
\noindent 5. field-sensitive 和 context-sensitive 对指针分析精度有什么影响。\\

\subsection*{四、Andersen 约束生成（20 分）}
\addcontentsline{toc}{subsection}{四、Andersen 约束生成（20 分）}
...
\noindent 1. 对每条语句写出 Andersen 约束。\\
\noindent 2. 写出初始 points-to 集。\\
\noindent 3. 说明 \texttt{*p = q} 的约束含义。\\
\noindent 4. 说明 \texttt{t = *p} 的约束含义。\\
\noindent 5. 说明为什么该分析是保守的 may analysis。\\

\subsection*{五、Andersen 图闭包推导（20 分）}
\addcontentsline{toc}{subsection}{五、Andersen 图闭包推导（20 分）}
...
\noindent 1. 构建约束图的初始边。\\
\noindent 2. 按闭包传播写出 \texttt{pts(p)}、\texttt{pts(q)}、\texttt{pts(r)}、\texttt{pts(s)}。\\
\noindent 3. 推导 \texttt{pts(a)}。\\
\noindent 4. 推导 \texttt{pts(t)}。\\
\noindent 5. 写出最终可能的别名关系。\\
...
\subsection*{一、调用图、函数摘要与上下文敏感解析}
\addcontentsline{toc}{subsection}{一、调用图、函数摘要与上下文敏感解析}


调用图：

\begin{center}
\includegraphics[width=0.72\linewidth]{figures/callgraph-14.png}
\end{center}
...
\subsection*{二、调用串与 CFL Reachability 解析}
\addcontentsline{toc}{subsection}{二、调用串与 CFL Reachability 解析}
...
\subsection*{三、指针别名分析基本概念答案}
\addcontentsline{toc}{subsection}{三、指针别名分析基本概念答案}
...
\subsection*{四、Andersen 约束生成答案}
\addcontentsline{toc}{subsection}{四、Andersen 约束生成答案}
...
\subsection*{五、Andersen 图闭包推导答案}

\begin{center}
\includegraphics[width=0.88\linewidth]{figures/andersen-points-to-14.png}
\end{center}

\addcontentsline{toc}{subsection}{五、Andersen 图闭包推导答案}
