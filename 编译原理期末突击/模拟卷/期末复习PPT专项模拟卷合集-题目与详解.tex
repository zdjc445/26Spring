...
\clearpage
\begin{center}
{\zihao{3}\bfseries 期末复习 PPT 专项模拟卷合集（题目与考试标准答案）}
\end{center}

\section*{卷面说明}

本合集只覆盖 \texttt{期末复习/} 下四份复习 PDF 已出现的内容：

\begin{itemize}
\item \texttt{期末复习一--词法解析-语法解析LL(1)文法(1).pdf}
\item \texttt{期末复习二--语法解析LR(1)文法(1).pdf}
\item \texttt{期末复习三--语义分析(1).pdf}
\item \texttt{期末复习四--中间代码生成(1).pdf}
\end{itemize}
...
\clearpage
\begin{center}
{\zihao{3}\bfseries 专项模拟卷 01}
\end{center}

\section*{A 卷题目}

\subsection*{一、词法分析：Thompson、子集构造、最小化（20 分）}
...
\noindent 1. 使用 Thompson 构造法构造等价 NFA。\par
\noindent 2. 使用子集构造法构造 DFA，写明每个 DFA 状态对应的 NFA 状态集合。\par
\noindent 3. 最小化 DFA。\par
...
\noindent 1. 消除左递归。\par
\noindent 2. 写 FIRST 和 FOLLOW。\par
\noindent 3. 写预测分析表。\par
\noindent 4. 判断消除左递归后的文法是否为 LL(1)。\par

\subsection*{三、LR(0)/SLR 分析（20 分）}
...
\noindent 1. 写出初始 LR(0) 项集 I0。\par
\noindent 2. 计算 GOTO(I0, id)、GOTO(I0, E)、GOTO(I0, T)、GOTO(I0, F)。\par
\noindent 3. 说明为什么含有 \texttt{E -> T .} 和 \texttt{T -> T . * F} 的状态在 LR(0) 中会出现移进-归约冲突。\par
\noindent 4. 说明 SLR 如何用 FOLLOW 集消除该冲突。\par

\subsection*{四、语义分析：属性与 SDD（20 分）}
...
\noindent 1. 说明综合属性和继承属性的区别。\par
\noindent 2. 为该文法设计一个 L 属性 SDD，把声明中的每个 \texttt{id} 加入符号表并记录类型。\par
\noindent 3. 说明为什么这个 SDD 是 L 属性的。\par

\subsection*{五、中间代码：短路求值（20 分）}

为下面语句生成三地址码，要求体现 \texttt{||} 和 \texttt{\&\&} 的短路求值，且 \texttt{\&\&} 优先级高于 \texttt{||}：
...
\begin{center}
\includegraphics[width=\textwidth]{nfa-thompson-abstar-abb.png}

\includegraphics[width=0.88\textwidth]{dfa-subset-abstar-abb.png}

\includegraphics[width=0.88\textwidth]{dfa-min-abstar-abb.png}
\end{center}
...
\clearpage
\begin{center}
{\zihao{3}\bfseries 专项模拟卷 02}
\end{center}

\section*{A 卷题目}

\subsection*{一、词法分析：密码规则正则表达式（20 分）}
...
\subsection*{三、LR(0) 项集族与 SLR 表（20 分）}
...
\noindent 1. 构造规范 LR(0) 项集族。\par
\noindent 2. 写出 GOTO 转移。\par
\noindent 3. 写 FOLLOW 集。\par
\noindent 4. 写 SLR 分析表中的 ACTION/GOTO 条目。\par

\subsection*{四、语义分析：S 属性定义（20 分）}
...
属性 \texttt{code} 表示后缀表达式。设计 S 属性 SDD，并计算输入：
...
\subsection*{五、中间代码：while 翻译（20 分）}
...
\clearpage
\begin{center}
{\zihao{3}\bfseries 专项模拟卷 03}
\end{center}

\section*{A 卷题目}

\subsection*{一、FIRST/FOLLOW 与 LL(1) 表（25 分）}
...
\noindent 1. 写出 FIRST 集。\par
\noindent 2. 写出 FOLLOW 集。\par
\noindent 3. 构造预测分析表。\par
\noindent 4. 判断该文法是否为 LL(1)。\par

\subsection*{二、LR(0) 与 SLR 的区别（15 分）}
...
\noindent 1. 说明该状态在 LR(0) 中为什么有移进-归约冲突。\par
\noindent 2. 若 \texttt{FOLLOW(E) = \{ +, ), \$ \}}，说明 SLR 在输入 \texttt{*} 时如何处理。\par

\subsection*{三、语义分析：属性分类与求值顺序（20 分）}
...
\noindent 1. 指出 \texttt{T.type} 和 \texttt{L.in} 分别是综合属性还是继承属性。\par
\noindent 2. 写出声明 \texttt{int x} 的属性求值顺序。\par
\noindent 3. 说明 \texttt{addType(id.name, L.in)} 的作用。\par

\subsection*{四、布尔表达式回填（25 分）}
...
\subsection*{五、SSA 中的 phi 函数（15 分）}
...
\subsection*{一、FIRST/FOLLOW 与 LL(1) 表答案}
...
表示如果下一个输入符号是 \texttt{*}，应该移进 \texttt{*}，继续识别乘法。

在 LR(0) 中，完成项 \texttt{E -> T .} 会在所有终结符上放置规约动作。因此在 \texttt{*} 上同时有：
...
因为 \texttt{*} 不在 \texttt{FOLLOW(E)} 中，所以 SLR 不在 \texttt{*} 上放置 \texttt{reduce E -> T}。因此 \texttt{*} 上只保留移进动作。
...
\texttt{addType(id.name, L.in)} 的作用：
