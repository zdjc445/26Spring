...
对 E' -> epsilon：
epsilon 产生式要看 FOLLOW(E')。
FOLLOW(E') = { ), $ }
所以 M[E', )] = E' -> epsilon，M[E', $] = E' -> epsilon。

对 T -> F T'：
FIRST(F T') = FIRST(F) = { (, id }
所以 M[T, (] = T -> F T'，M[T, id] = T -> F T'。
...
对 T' -> epsilon：
epsilon 产生式要看 FOLLOW(T')。
FOLLOW(T') = { +, ), $ }
所以 M[T', +] = T' -> epsilon，
M[T', )] = T' -> epsilon，M[T', $] = T' -> epsilon。
