...
\item \texttt{Slides/Slides/handout/1 Regex NFA DFA.pdf}
...
\item \texttt{Exam/过时/2021-exam-A.pdf} 题目 1
\end{itemize}


\subsection{一、先记住考试目标}
...
\begin{itemize}
\item 正则表达式描述一类字符串。
\item NFA 像“分身试路”，同一输入能走多条路，还能走 epsilon 边。
\item DFA 像“把全部分身装进一个集合”，每个 DFA 状态就是一组 NFA 状态。
\item 最小化就是合并“之后对所有输入表现相同”的 DFA 状态。
\end{itemize}
...
\begin{itemize}
\item Lexer：字符流变 token 流。
\item Parser：token 流变语法树。
\item Semantic Analysis：查类型、作用域、声明使用。
\item IR：生成中间表示。
\item Optimization：改写 IR，保持语义不变。
\item Code Generation：生成目标机器代码。
\end{itemize}


\subsection{三、字符串、语言、正则}
...
\item \texttt{A+ = A A*}。
\item 正则语言对并、交、连接、补、反转、Kleene star 封闭。
\end{itemize}


\subsection{四、DFA 与 NFA 定义}
