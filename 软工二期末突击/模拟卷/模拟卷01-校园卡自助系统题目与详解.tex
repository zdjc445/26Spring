...
\tableofcontents
\newpage
\section{软件工程与计算 II 期末模拟卷 01：校园卡自助系统（题目与详解）}
...
\item 针对 \texttt{充值与查询充值记录} 写出 Service 接口、Repository 方法声明、Controller 调用 Service 的依赖注入代码片段，写出包名。
\end{enumerate}
...
系统中 \texttt{card、recharge、payment、record、user} 包之间存在直接互相访问数据对象和调用内部方法的现象。
...
业务中存在变化点：新增优惠券类型和支付方式。请说明使用 \texttt{Strategy} 如何设计。
...
某同学把 \texttt{password change doPost with complex boolean expression} 写在一个方法中，方法包含大量条件分支、魔法数字和对数据访问对象的直接调用。
...
针对 \texttt{充值与查询充值记录} 的核心方法：


\begin{enumerate}
\item 说明如何计算圈复杂度。
\item 设计满足 \texttt{条件覆盖} 的测试用例集。
\item 补充一个异常路径测试用例。
\end{enumerate}
...
针对 \texttt{充值页面金额输入框、支付方式选择、确认按钮、充值结果提示}，从 HCI 原则角度分析优点、不足和改进建议。至少写 5 点。
...
\begin{itemize}
\item 学生提交充值请求，系统校验校园卡状态，调用支付平台，支付成功后更新余额并保存充值记录。
\item 学生查询充值记录，系统按校园卡号返回充值时间、金额、渠道和状态。
\end{itemize}
...
\begin{itemize}
\item 业务需求：建设 校园卡自助系统，提升 充值与查询充值记录 的处理效率，降低人工处理成本。
\item 用户需求：学生 希望通过系统自助完成 充值与查询充值记录 并查看处理结果。
...
\begin{tikzpicture}
\node[umlbox] (user) at (0,2.3) {User};
\node[umlbox] (campusCard) at (4,2.3) {CampusCard};
\node[umlbox] (bankCard) at (0,0.4) {BankCard};
\node[umlbox] (rechargeRecord) at (8,2.3) {RechargeRecord};
\node[umlbox] (paymentOrder) at (12,2.3) {PaymentOrder};
\node[umlbox] (transactionDetail) at (4,0.4) {TransactionDetail};
\draw (user) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (campusCard);
\draw (user) -- node[left,pos=0.12,font=\scriptsize] {1} node[left,pos=0.88,font=\scriptsize] {0..*} (bankCard);
\draw (campusCard) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (rechargeRecord);
\draw (rechargeRecord) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {1} (paymentOrder);
\draw (campusCard) -- node[right,pos=0.12,font=\scriptsize] {1} node[right,pos=0.88,font=\scriptsize] {0..*} (transactionDetail);
\end{tikzpicture}}
\end{center}
...
\ExamPhysicalPackageDiagram{card \quad recharge \quad payment \quad record \quad user}
...
\ExamDetailedSequenceDiagram{充值与查询充值记录}{RechargeService}{recharge}{CampusCardRepository}{RechargeRecordRepository}
...
\item 充值与查询充值记录 适合委托式控制，因为它包含校验、规则计算、外部接口、持久化和结果返回。
\end{itemize}
...
\ExamStrategyDiagram{Context}{Strategy}{FixedAmountCouponStrategy}{RateCouponStrategy}{WechatPaymentStrategy}
...
\begin{itemize}
\item 系统状态可见：充值页面金额输入框、支付方式选择、确认按钮、充值结果提示 中的提交、处理中、成功、失败状态都要明确显示。
