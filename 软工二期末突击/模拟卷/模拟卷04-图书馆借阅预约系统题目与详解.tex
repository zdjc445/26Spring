...
\tableofcontents
\newpage
\section{软件工程与计算 II 期末模拟卷 04：图书馆借阅预约系统（题目与详解）}
...
\item 针对 \texttt{借书、还书与预约} 写出 Service 接口、Repository 方法声明、Controller 调用 Service 的依赖注入代码片段，写出包名。
\end{enumerate}
...
系统中 \texttt{catalog、loan、reservation、notice、borrower} 包之间存在直接互相访问数据对象和调用内部方法的现象。
...
业务中存在变化点：新增通知渠道。请说明使用 \texttt{Observer} 如何设计。
...
某同学把 \texttt{borrow validation mixed with notice sending and persistence} 写在一个方法中，方法包含大量条件分支、魔法数字和对数据访问对象的直接调用。
...
针对 \texttt{借书、还书与预约} 的核心方法：


\begin{enumerate}
\item 说明如何计算圈复杂度。
\item 设计满足 \texttt{等价类与边界值测试} 的测试用例集。
\item 补充一个异常路径测试用例。
\end{enumerate}
...
针对 \texttt{检索书名、预约按钮、可借状态提示、借阅清单}，从 HCI 原则角度分析优点、不足和改进建议。至少写 5 点。
...
\begin{itemize}
\item 借书人检索图书并提交借阅请求，系统检查可借副本，登记借出记录。
\item 借书人预约已借出图书，系统记录预约优先级并在可借时通知。
\end{itemize}
...
\begin{itemize}
\item 业务需求：建设 图书馆借阅预约系统，提升 借书、还书与预约 的处理效率，降低人工处理成本。
\item 用户需求：借书人 希望通过系统自助完成 借书、还书与预约 并查看处理结果。
...
\begin{tikzpicture}
\node[umlbox] (borrower) at (0,2.6) {Borrower};
\node[umlbox] (loan) at (4,2.6) {Loan};
\node[umlbox] (copy) at (8,2.6) {BookCopy};
\node[umlbox] (title) at (12,2.6) {BookTitle};
\node[umlbox] (reservation) at (4,0.4) {Reservation};
\node[umlbox] (notice) at (4,-1.5) {OverdueNotice};
\node[umlbox] (librarian) at (0,0.4) {Librarian};
\draw (borrower) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (loan);
\draw (loan) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {1..*} (copy);
\draw (copy) -- node[above,pos=0.12,font=\scriptsize] {*} node[above,pos=0.88,font=\scriptsize] {1} (title);
\draw (borrower) -- node[below,pos=0.12,font=\scriptsize] {1} node[below,pos=0.88,font=\scriptsize] {0..*} (reservation);
\draw (reservation) -- node[below,pos=0.12,font=\scriptsize] {*} node[below,pos=0.88,font=\scriptsize] {1} (title);
\draw (notice) -- node[right,pos=0.12,font=\scriptsize] {*} node[right,pos=0.88,font=\scriptsize] {1} (loan);
\end{tikzpicture}}
\end{center}
...
\ExamPhysicalPackageDiagram{catalog \quad loan \quad reservation \quad notice \quad borrower}
...
\ExamDetailedSequenceDiagram{借书、还书与预约}{LoanService}{borrowBooks}{BookCopyRepository}{ReservationRepository}
...
\item 借书、还书与预约 适合委托式控制，因为它包含校验、规则计算、外部接口、持久化和结果返回。
\end{itemize}
...
\begin{itemize}
\item 系统状态可见：检索书名、预约按钮、可借状态提示、借阅清单 中的提交、处理中、成功、失败状态都要明确显示。
