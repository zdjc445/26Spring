...
\begin{tikzpicture}
\node[umlbox] (consumer) at (0,2.6) {Consumer};
\node[umlbox] (paymentCode) at (4,3.6) {PaymentCode};
\node[umlbox] (point) at (4,1.6) {PointAccount};
\node[umlbox] (record) at (4,-0.4) {PaymentRecord};
\node[umlbox] (merchant) at (0,-0.4) {Merchant};
\node[umlbox] (method) at (8,0.6) {PaymentMethod};
\node[umlbox] (coupon) at (8,-1.4) {Coupon};
\draw (consumer) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {1} (paymentCode);
\draw (consumer) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {1} (point);
\draw (consumer) -- node[right,pos=0.12,font=\scriptsize] {1} node[right,pos=0.88,font=\scriptsize] {0..*} (record);
\draw (merchant) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (record);
\draw (record) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {1} (method);
\draw (record) -- node[below,pos=0.12,font=\scriptsize] {0..1} node[below,pos=0.88,font=\scriptsize] {1} (coupon);
\end{tikzpicture}}
\end{center}
...
\ExamPhysicalPackageDiagram{payment \quad coupon \quad point \quad account \quad merchant}
...
\ExamDetailedSequenceDiagram{扫码付款与积分计算}{PaymentService}{pay}{PaymentRecordRepository}{PointAccountRepository}
...
\ExamStrategyDiagram{Context}{Strategy}{LevelARewardStrategy}{LevelBRewardStrategy}{LevelSRewardStrategy}
