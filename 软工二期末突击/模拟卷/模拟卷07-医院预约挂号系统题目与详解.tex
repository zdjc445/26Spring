...
\tableofcontents
\newpage
\section{软件工程与计算 II 期末模拟卷 07：医院预约挂号系统（题目与详解）}
...
\item 针对 \texttt{预约挂号与退号} 写出 Service 接口、Repository 方法声明、Controller 调用 Service 的依赖注入代码片段，写出包名。
\end{enumerate}
...
系统中 \texttt{patient、doctor、appointment、payment、refund} 包之间存在直接互相访问数据对象和调用内部方法的现象。
...
业务中存在变化点：替换医院 HIS 数据源。请说明使用 \texttt{Abstract Factory} 如何设计。
...
某同学把 \texttt{appointment booking mixes payment, slot locking and database access} 写在一个方法中，方法包含大量条件分支、魔法数字和对数据访问对象的直接调用。
...
针对 \texttt{预约挂号与退号} 的核心方法：


\begin{enumerate}
\item 说明如何计算圈复杂度。
\item 设计满足 \texttt{状态转换测试} 的测试用例集。
\item 补充一个异常路径测试用例。
\end{enumerate}
...
针对 \texttt{科室选择、医生排班、号源状态、支付倒计时、退号入口}，从 HCI 原则角度分析优点、不足和改进建议。至少写 5 点。
...
\begin{itemize}
\item 患者选择科室、医生和号源，系统锁定号源并创建预约订单。
\item 患者退号，系统校验退号规则，生成退款记录并释放号源。
\end{itemize}
...
\begin{itemize}
\item 业务需求：建设 医院预约挂号系统，提升 预约挂号与退号 的处理效率，降低人工处理成本。
\item 用户需求：患者 希望通过系统自助完成 预约挂号与退号 并查看处理结果。
...
\begin{tikzpicture}
\node[umlbox] (patient) at (0,2.6) {Patient};
\node[umlbox] (appointment) at (4,2.6) {Appointment};
\node[umlbox] (doctor) at (8,2.6) {Doctor};
\node[umlbox] (department) at (12,2.6) {Department};
\node[umlbox] (timeslot) at (4,0.5) {TimeSlot};
\node[umlbox] (payment) at (0,0.5) {PaymentOrder};
\node[umlbox] (refund) at (8,0.5) {RefundRecord};
\draw (patient) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (appointment);
\draw (doctor) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (appointment);
\draw (doctor) -- node[above,pos=0.12,font=\scriptsize] {*} node[above,pos=0.88,font=\scriptsize] {1} (department);
\draw (appointment) -- node[right,pos=0.12,font=\scriptsize] {1} node[right,pos=0.88,font=\scriptsize] {1} (timeslot);
\draw (appointment) -- node[below,pos=0.12,font=\scriptsize] {1} node[below,pos=0.88,font=\scriptsize] {0..1} (payment);
\draw (appointment) -- node[below,pos=0.12,font=\scriptsize] {1} node[below,pos=0.88,font=\scriptsize] {0..1} (refund);
\end{tikzpicture}}
\end{center}
...
\ExamPhysicalPackageDiagram{patient \quad doctor \quad appointment \quad payment \quad refund}
...
\ExamDetailedSequenceDiagram{预约挂号与退号}{AppointmentService}{bookAppointment}{AppointmentRepository}{DoctorScheduleRepository}
...
\item 预约挂号与退号 适合委托式控制，因为它包含校验、规则计算、外部接口、持久化和结果返回。
\end{itemize}
...
\begin{itemize}
\item 系统状态可见：科室选择、医生排班、号源状态、支付倒计时、退号入口 中的提交、处理中、成功、失败状态都要明确显示。
