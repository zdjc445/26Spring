# 软件工程与计算 II 期末模拟卷 05：移动支付积分系统（题目与详解）
...
2. 针对 `扫码付款与积分计算` 写出 Service 接口、Repository 方法声明、Controller 调用 Service 的依赖注入代码片段，写出包名。
...
系统中 `payment、coupon、point、account、merchant` 包之间存在直接互相访问数据对象和调用内部方法的现象。
...
业务中存在变化点：新增 S 级会员和新的积分规则。请说明使用 `Strategy` 如何设计。
...
某同学把 `payment method selected by switch and membership level type code` 写在一个方法中，方法包含大量条件分支、魔法数字和对数据访问对象的直接调用。
...
针对 `付款码页面、金额确认、密码输入、支付结果反馈`，从 HCI 原则角度分析优点、不足和改进建议。至少写 5 点。
...
- 消费者扫描商家二维码并输入金额，系统校验付款方式，支付成功后生成付款记录。
- 系统根据会员等级和支付金额计算积分并更新积分账户。

需求层次：

- 业务需求：建设 移动支付积分系统，提升 扫码付款与积分计算 的处理效率，降低人工处理成本。
- 用户需求：消费者 希望通过系统自助完成 扫码付款与积分计算 并查看处理结果。
...
flowchart LR
    ...
    subgraph Server["服务器端"]
        ...
        modules["modules: payment coupon point account merchant"]
        database
    end
...
sequenceDiagram
    ...
    participant PaymentRecordRepository
    participant StrategyDomain as Strategy/Domain
    participant ExternalService
    participant PointAccountRepository
    ...
    PaymentRecordRepository-->>PaymentService: domain object
    PaymentService->>StrategyDomain: check rules and calculate result
    PaymentService->>ExternalService: call external interface if needed
    ExternalService-->>PaymentService: external result
...
- 扫码付款与积分计算 适合委托式控制，因为它包含校验、规则计算、外部接口、持久化和结果返回。
...
- 系统状态可见：付款码页面、金额确认、密码输入、支付结果反馈 中的提交、处理中、成功、失败状态都要明确显示。
