...
flowchart LR
    ...
    subgraph Server["服务器端"]
        ...
        modules["modules: catalog loan reservation notice borrower"]
        database
    end
...
sequenceDiagram
    ...
    BookCopyRepository-->>LoanService: domain object
    LoanService->>StrategyDomain: check rules and calculate result
    LoanService->>ExternalService: call external interface if needed
    ExternalService-->>LoanService: external result
...
classDiagram
    ...
    NoticeSubject ..> NoticeObserver : notifies
    NoticeObserver <|.. EmailNoticeDisplay
    NoticeObserver <|.. SmsNoticeDisplay
    NoticeObserver <|.. AppNoticeDisplay
```
