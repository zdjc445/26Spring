...
\begin{tikzpicture}
\node[umlbox] (user) at (0,2.6) {User};
\node[umlbox] (reservation) at (4,2.6) {Reservation};
\node[umlbox] (device) at (8,2.6) {Device};
\node[umlbox] (lab) at (12,2.6) {Lab};
\node[umlbox] (ticket) at (8,0.5) {RepairTicket};
\node[umlbox] (maintenance) at (12,0.5) {MaintenanceRecord};
\node[umlbox] (notification) at (0,0.5) {Notification};
\draw (user) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (reservation);
\draw (reservation) -- node[above,pos=0.12,font=\scriptsize] {*} node[above,pos=0.88,font=\scriptsize] {1} (device);
\draw (device) -- node[above,pos=0.12,font=\scriptsize] {*} node[above,pos=0.88,font=\scriptsize] {1} (lab);
\draw (device) -- node[right,pos=0.12,font=\scriptsize] {1} node[right,pos=0.88,font=\scriptsize] {0..*} (ticket);
\draw (ticket) -- node[above,pos=0.12,font=\scriptsize] {1} node[above,pos=0.88,font=\scriptsize] {0..*} (maintenance);
\draw (notification) -- node[left,pos=0.12,font=\scriptsize] {*} node[left,pos=0.88,font=\scriptsize] {1} (user);
\end{tikzpicture}}
\end{center}
...
\ExamPhysicalPackageDiagram{device \quad reservation \quad repair \quad maintenance \quad notification}
...
\ExamDetailedSequenceDiagram{设备预约、归还与报修}{DeviceReservationService}{reserve}{DeviceRepository}{ReservationRepository}
