...
\section{软件工程与计算 II 期末名词解释}


\subsection{使用说明}


本文件按 PPT/ 课件和 软工二课程整理/课程框架与重点名词.md 重新梳理。背诵顺序不按原来的零散名词走，而按软件工程活动链路走：
...
\begin{itemize}
\item 先背每章的“记忆链”，知道概念之间的上下位关系。
\item 再背名词定义，答题时按“是什么 + 解决什么问题 + 常见关键词”写。
\item 最后背卷面判定句，用来区分相似概念。
\end{itemize}
...
\begin{itemize}
\item 需求工程：把现实世界问题变成可验证、可追踪的规格。
\item 体系结构：决定系统大的部件、连接方式和约束。
\item 详细设计：把架构细化成类、方法、接口和对象协作。
\item 软件构造：把设计变成可运行、可测试、可维护的代码。
\item 项目管理：在范围、时间、成本和质量之间做控制。
\item AI 协作：生成初稿可以交给 AI，工程判断必须由人负责。
\end{itemize}
...
\subsubsection*{Build-and-Fix Model}
...
\subsubsection*{Spec-Driven Development}


规格驱动开发。把规格说明作为首要产物，后续设计、任务拆分、代码和测试从规格推导。


\section{五、体系结构与人机交互：把规格组织成系统}
...
\subsubsection*{体系结构风格}


一组设计决策，规定组件类型、连接方式和拓扑约束。


\subsubsection*{主程序/子程序风格}


以过程调用组织系统。优点是流程清楚、控制强；缺点是调用耦合强，接口变化影响大。


\subsubsection*{面向对象风格}
...
\subsubsection*{Red-Green-Refactor}
