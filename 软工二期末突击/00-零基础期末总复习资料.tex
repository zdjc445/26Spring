...
\tableofcontents
\newpage
\section{软件工程与计算 II 零基础期末突击总复习资料}


\subsection{使用方法}


这份资料只服务一个目标：在期末大题中稳定写出步骤分。往年卷完整结构以 \texttt{Exam/2020.pdf}、\texttt{Exam/2022.pdf} 为主，2025 卷加入了 Spring Boot 三层代码、AI 情境、排课系统和代码改造题。Review 资料补充了旧题中的过程模型、质量保障、配置管理、HCI、测试与维护。
...
\subsection{四、用例与分析模型}


\subsubsection{用例图四要素}
...
\subsection{五、体系结构题模板}


\subsubsection{Web 物理包图}
...
答题要点：客户端必须出现 \texttt{html}、\texttt{css}、\texttt{javascript} 和展示层；服务器端必须出现逻辑层和数据层；跨网络连线标注 HTTP、REST API、JSON。


\subsubsection{Spring Boot 三层职责}
...
\subsection{六、详细设计题模板}


\subsubsection{Controller-Service-Repository 顺序图}
...
\subsubsection{Abstract Factory / DAO}
...
判定节点包括：\texttt{if}、\texttt{while}、\texttt{for}、\texttt{case}、复杂布尔表达式中的独立条件按题目要求处理。
